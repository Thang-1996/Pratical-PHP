...
<body>
<form method="post" action="{{url("update-book/{$books->__get("id")}")}}">
    @method("PUT")
    @csrf
    <div class="form-group">
        <label for="exampleInputEmail1">Title</label>
        <input type="text" value="{{$books->__get("title")}}" name="title" class="form-control @error("title") is-invalid @enderror" placeholder="New Title">
        @error("title")
